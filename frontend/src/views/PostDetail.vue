  ...
  <div class="post-details container">
    <div class="post-content rounded">
      ...
      <div class="post-reactions">
        ...
        <span :key="this.reactions.dislikes.length">{{
      ...
      >
        ...
        <button class="btn btn-dark" @click="gotoUpdatePost">
          Editer ce post
        </button>
      </div>
    </div>
    <div class="post-comments">
      ...
      <div
        :id="comment.id"
        class="post-comments-item my-3 rounded"
        v-for="comment in comments"
        :key="comment.id"
      >
        <div class="post-comments-header">
          #{{ comment.id }} - Posté le : {{ comment.date_posted }} par
...
export default {
  ...
  methods: {
    getPost(id) {
      postsQueries
        ...
        .then((response) => {
          ...
          this.post.date_posted = moment(this.post.date_posted)
            .utc()
    ...
    deletePost() {
      ...
      postsQueries
        .delete(this.post.id, this.post.userId)
        .then((response) => {
      ...
      postsQueries
        ...
        .then((response) => {
          response.data.forEach((comment) => {
            comment.date_posted = moment(comment.date_posted)
              .utc()
              .format("DD-MM-YYYY à hh:mm:ss");
    ...
    saveComment() {
      ...
      postsQueries
        .createComment(data.postId, data)
        .then((response) => {
    ...
    likePost() {
      ...
      if (this.checkLikes()) {
        ...
        postsQueries
          .createReaction(data.postId, data)
          .then((response) => {
            console.log(response.data);
            this.getReactions(this.$route.params.id);
            document.getElementById("btn-like").innerHTML =
              '<i class="far fa-thumbs-up"></i>';
      ...
      if (this.checkDislikes()) {
        ...
        postsQueries
          ...
          .then((response) => {
            ...
            this.getReactions(this.$route.params.id);
            if (this.checkDislikes()) {
              document.getElementById("btn-dislike").innerHTML =
                '<i class="far fa-thumbs-down"></i>';
    ...
    checkLikes() {
      if (
        this.reactions.likes.some(
          (el) =>
            el.postId == this.post.id &&
